...
SELECT PROD_MAKE, PROD_MODEL, CI.CI_ID, CI.CI_QTY, PP_AMOUNT, PROD_IMG, COUNT(P.PROD_ID) AS PRODUCT_COUNT FROM CART_ITEM AS CI INNER JOIN CART AS C ON CI.CART_ID = C.CART_ID INNER JOIN [USER] AS U ON C.USER_ID = U.USER_ID INNER JOIN PRODUCT AS P ON CI.PROD_ID = P.PROD_ID INNER JOIN PRODUCT_PRICE AS PP ON P.PROD_ID = PP.PROD_ID WHERE C.USER_ID = 1 GROUP BY PROD_MAKE, PROD_MODEL, CI.CI_QTY, PP_AMOUNT, PROD_IMG, CI.CI_ID;
