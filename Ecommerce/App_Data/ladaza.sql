...
-- JOIN FOR ALL USER PRODUCTS INSIDE HIS/HER CART
SELECT * FROM [USER] AS U
	INNER JOIN 
	CART AS C ON U.USER_ID = C.USER_ID
	INNER JOIN 
	CART_ITEM AS CI ON C.CART_ID = CI.CART_ID
	INNER JOIN 
	PRODUCT AS P ON CI.PROD_ID = P.PROD_ID

-- JOIN USER FOR CART DISPLAY
SELECT USER_FNAME, USER_LNAME, PROD_MAKE, PROD_MODEL, PP_AMOUNT, PROD_DESC, PP_AMOUNT * 2 AS "TOTAL PRICE" 
	FROM 
	CART_ITEM AS CI
	INNER JOIN
	CART AS C
	ON CI.CART_ID = CI.CI_ID
	INNER JOIN
	[USER] AS U
...
SELECT PROD_MAKE, PROD_MODEL, PP_AMOUNT, PROD_DESC
    FROM CART_ITEM AS CI
    INNER JOIN CART AS C ON CI.CART_ID = CI.CI_ID
    INNER JOIN [USER] AS U ON C.USER_ID = U.USER_ID
    INNER JOIN PRODUCT AS P ON CI.PROD_ID = P.PROD_ID
    INNER JOIN PRODUCT_PRICE AS PP ON P.PROD_ID = PP.PROD_ID
    WHERE U.USER_ID = 1 AND C.CART_ID = 1 AND CI.CI_ID = 1 AND P.PROD_ID = 3

-- THIS QUERY IS NOT SPECIFIC. THIS IS INTENDED TO RETRIEVE ALL PRODUCTS TO BE DISPLAYED IN THE CART.
SELECT PROD_MAKE, PROD_MODEL, CI.CI_QTY, PP_AMOUNT, PROD_DESC
    FROM CART_ITEM AS CI
    INNER JOIN CART AS C ON CI.CART_ID = C.CART_ID
    INNER JOIN [USER] AS U ON C.USER_ID = U.USER_ID
    INNER JOIN PRODUCT AS P ON CI.PROD_ID = P.PROD_ID
    INNER JOIN PRODUCT_PRICE AS PP ON P.PROD_ID = PP.PROD_ID
    WHERE C.USER_ID = 1

-- MODIFIED CART CONTENTS
SELECT PROD_MAKE, PROD_MODEL, CI.CI_QTY, CI.CI_ID, PP_AMOUNT, PROD_IMG, COUNT(P.PROD_ID) AS PRODUCT_COUNT, SUM(PP_AMOUNT) AS PRODUCT_TOTAL 
	FROM CART_ITEM AS CI
	INNER JOIN 
	CART AS C ON CI.CART_ID = C.CART_ID 
	...
	C.USER_ID = 2
	GROUP BY 
	PROD_MAKE, PROD_MODEL, CI.CI_QTY, PP_AMOUNT, PROD_IMG, CI.CI_ID;

SELECT SUM(CI.CI_QTY * PP.PP_AMOUNT) AS PRODUCT_TOTAL 
FROM CART_ITEM AS CI
INNER JOIN CART AS C ON CI.CART_ID = C.CART_ID 
INNER JOIN [USER] AS U ON C.USER_ID = U.USER_ID 
INNER JOIN PRODUCT AS P ON CI.PROD_ID = P.PROD_ID 
...
SELECT PROD_MAKE, PROD_MODEL, CI.CI_ID, CI.CI_QTY, PP_AMOUNT, PROD_IMG, COUNT(P.PROD_ID) AS PRODUCT_COUNT 
	FROM 
	CART_ITEM AS CI 
	INNER JOIN 
	CART AS C ON CI.CART_ID = C.CART_ID 
	...
	PRODUCT AS P ON CI.PROD_ID = P.PROD_ID 
	INNER JOIN 
	PRODUCT_PRICE AS PP ON P.PROD_ID = PP.PROD_ID 
	WHERE 
	C.USER_ID = 1 
	GROUP BY 
	PROD_MAKE, PROD_MODEL, CI.CI_QTY, PP_AMOUNT, PROD_IMG, CI.CI_ID;

SELECT P.PROD_ID, P.PROD_MAKE, P.PROD_MODEL, P.PROD_WARRANTY, P.PROD_IMG, 
P.PROD_DESC, PQ.PQ_QTY, PP.PP_AMOUNT, PA.PA_DATE, PL.PL_ID, PL.PL_DATE, 
PL.PL_TIME, D.D_ID, D.D_NAME, D.D_TIME_ENTERED, D.D_DATE_ENTERED 
FROM PRODUCT AS P 
INNER JOIN 
PRODUCT_QUANTITY AS PQ 
...
INNER JOIN PRODUCT_PRICE AS PP 
ON P.PROD_ID = PP.PROD_ID 
INNER JOIN PRODUCT_ACQUISITION AS PA 
ON P.PROD_ID = PA.PROD_ID 
INNER JOIN PRODUCT_LOG AS PL 
